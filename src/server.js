...
const { PORT = 5001, REDIS_URL } = process.env;
...
async function init() {
  try {
    ...
    // Start the server
    app.listen(PORT, () => logger.info("Internal Server started", { port: PORT }));
    
  } catch (err) {
    logger.error('Failed to initialize internal server:', { error: err.message });
