...
class RedisService {
  ...
  async deleteKeysContaining(substring) {
    if (!this.client) return;
    try {
      ...
      // Use logger to avoid console statements if linting disallows console
      logger.info(`Deleted ${totalDeleted} keys containing "${substring}"`);
    } catch (_err) {
      // fail silently
    }
  }
